...
while current_distance != desired_distance:
    ...
    if current_distance > 10:
        print('Speed: The arm is moving faster')
        current_distance -= 3
    elif current_distance > 5:
        print('Speed: The arm is moving at the same speed')
        current_distance -= 3
    elif current_distance > desired_distance and current_distance < 5:
        if current_distance > 3:
            print('Speed: The arm is reducing the speed')
            current_distance -= 3
        else:
            print('Speed: The arm is reducing the speed')
            current_distance = current_distance - (current_distance - desired_distance) / 2
    else:
        print('Speed: The arm has stopped and is moving backwards')
        current_distance += 1

print('The arm has reached the desired distance: ' + str(desired_distance) + ' cm')